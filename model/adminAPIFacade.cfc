<cfcomponent name="adminAPIFacade" output="false" hint="a wrapper for admiapi functions">
	<cffunction name="init" access="public" output="false" returntype="adminAPIFacade">
		...
		<cftry>
			...
			<cfcatch type="cfadminapiSecurityError"></cfcatch>			
		</cftry>
		<cfreturn this />
	</cffunction>
	...
	<cffunction name="setDatasources" access="public" output="false" returntype="void">
		...
		<cfloop from="1" to="#arrayLen(sortedDSNs)#" index="i">
			...
			<cfif len(thisType)>
				<cfset objDatasource = createObject("component","datasource.datasource").init(dsns[thisDSN].name,thisType,dsns[thisDSN].driver) />
	...
	<cffunction name="getDatasource" access="public" output="false" returntype="nextgenerator.model.datasource.datasource">
